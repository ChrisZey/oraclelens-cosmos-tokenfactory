...
HTTPS_GIT := github.com/rollchains/oraclelens.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=oraclelens \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=oraclelensd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/oraclelensd ./cmd/oraclelensd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/oraclelensd.exe ./cmd/oraclelensd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/oraclelensd
...
draw-deps:
	...
	@goviz -i ./cmd/oraclelensd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c oraclelens --local -f chains.yaml
endif
...
set-testnet-configs:
	oraclelensd config set client chain-id chainid-1
	oraclelensd config set client keyring-backend test
	oraclelensd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | oraclelensd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | oraclelensd keys add acc1 --recover`
