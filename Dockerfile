...
# then log output of file /code/bin/oraclelensd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/oraclelensd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/oraclelensd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/oraclelensd /usr/bin/oraclelensd
...
CMD ["/usr/bin/oraclelensd", "version"]
